#' Load presence records from VIC database
#'
...
load_pres_bg_data_VIC_improper_2 <- function(dir_path,
                                             species,
                                             region = "all",
                                             save.map = TRUE,
                                             map.directory = "."){
  ...
  VIC_data <- read.csv(sprintf("%s/VIC_%s.csv",
                               dir_path,
                               gsub(" ",
                                    "_",
                                    tolower(species))),
  ...
  df <- data.frame("ID" = seq_len(nrow(VIC_data)),
                   "Origin" = "VIC",
                   "Species" = species,
                   "Longitude" = VIC_data$Longitude.GDA94,
                   "Latitude" = VIC_data$Latitude.GDA94,
                   #add date for duplicate processing
                   "Date" = lubridate::as_date(VIC_data$Survey.Start.Date,
                                               format = "%d/%m/%Y"),
                   "Basis.of.Record" = VIC_data$Type.of.Record,
                   "Locality" = VIC_data$Site.Location.Description,
                   "Institute" = VIC_data$Observer,
                   "Collection" = VIC_data$Project.ID,
                   "Coordinate.Uncertainty.in.Metres" = VIC_data$Accuracy,
                   stringsAsFactors = FALSE)
