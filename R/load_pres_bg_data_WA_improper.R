#' Load presence records from WA database
#'
...
load_pres_bg_data_WA_improper <- function(dir_path,
                                          species,
                                          region = "all",
                                          save.map = TRUE,
                                          map.directory = "."){
  ...
  WA_data <- read.csv(sprintf("%s/WA_%s.csv",
                              dir_path,
                              gsub(" ",
                                   "_",
                                   tolower(species))),
  ...
  WA_data$DATE_COMBINED <- sprintf("%s-%s-%s",
                                   WA_data$COLDATEY,
                                   WA_data$COLDATEM,
                                   WA_data$COLDATED)
  ...
  df <- data.frame("ID" = seq_len(nrow(WA_data)),
                   "Origin" = "WA",
                   "Species" = species,
                   "Longitude" = WA_data$LONGITUDE,
                   "Latitude" = WA_data$LATITUDE,
                   #add date for duplicate processing
                   "Date" = lubridate::as_date(WA_data$DATE_COMBINED),
                   "Basis.of.Record" = NA,
                   "Locality" = WA_data$SITE_NAME,
                   "Institute" = WA_data$SOU_CODE,
                   "Collection" = NA,
                   "Coordinate.Uncertainty.in.Metres" = WA_data$ACCURACY_M,
                   stringsAsFactors = FALSE)
